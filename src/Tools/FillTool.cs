...
namespace linerider.Tools
{
    ...
    {
        public FillTool(GameCanvas gameCanvas)
        {
            _canvas = gameCanvas;
        }
        ...
        public override void OnMouseUp(Vector2d pos)
        {
            if (_drawingbox)
            {
                ...
                else
                {
                    ...
                    _selectionbox = GetBoxFromSelected(_selection);
                    _boxselection.Clear();
                    Debug.WriteLine("Selected " + _selection.Count + " lines with the FillTool");
                    if (_selection.Count>1)
                    {
                        Debug.WriteLine("Opening FillTool window!");
                        _canvas.ShowFillToolWindow(_selection);
                    }
                }
